...
class MaxCUTModel(BaseModel):
    def __init__(self, cfg):
        ...
        self.netF = networks.PatchSampleFv2(use_mlp=True, nc=cfg.netF_nc, gpu_ids=self.gpu_ids)
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        feat_q_pool, sample_ids = self.netF(feat_q, self.cfg.num_patches, None)
        total_nce_loss = 0.0
        for (f_q, f_k, sample_id, crit) in zip(feat_q_pool, feat_k_pool, sample_ids, self.criterionNCE):
            loss = crit(f_q, f_k, sample_id) * self.cfg.lambda_NCE
